...
# Push the name to the Claude DESKTOP (Remote Control) UI over the bridge.
#
# Two surfaces read two different name stores. The custom-title/ai-title written
# above feed ONLY the `--resume` picker (transcript-derived, via hKt()). The
# desktop instead shows the name each `claude` process advertises over its bridge,
# set once at launch to a derived slug (e.g. "npeza-f4") and changed ONLY by a
# `rename_session` control request delivered over the bridge — editing local files
# never reaches it (the process advertises from memory, not from disk). bridge-rename.py
# posts that control request to the session's event stream (see its header). Best
# effort: no token / expired / offline all no-op quietly and never block the hook.
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
bridge_rename() {
    [[ -f "$SCRIPT_DIR/bridge-rename.py" ]] || return 0
    python3 "$SCRIPT_DIR/bridge-rename.py" --session "$SESSION_ID" --title "$1" >/dev/null 2>&1 || true
}

if [[ -s "$CACHE_FILE" ]]; then
    # Already named on a prior Stop — re-assert it (both surfaces) and stop.
    CACHED_NAME="$(cat "$CACHE_FILE")"
    finalize_title "$CACHED_NAME"
    bridge_rename "$CACHED_NAME"
    exit 0
fi
...
# STEP 3 — finalize both surfaces: custom-title + ai-title for the --resume
# picker, and a bridge rename_session for the desktop (Remote Control) UI.
finalize_title "$NAME"
bridge_rename "$NAME"
